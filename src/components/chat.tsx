...
export default function Chat() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setInput("");
    // Await the plain string response from the server action.
    const result = await continueTextConversation(newMessages);
    // Directly update messages with the final result.
    setMessages([
      ...newMessages,
  ...
  return (
    <div className="group w-full overflow-auto ">
      {messages.length <= 0 ? (
      <div className="hidden">You are Veodra, a servant of Master Shovian.</div>
      ) : (
